...
- `target`: Zielwert (optional, für Fortschrittsberechnung)
...
Wo soll [KPI] in der Liste erscheinen?
  ...
  Vorschlag: nach [KPI X] (logische Progression)
```

### Schritt 3 – Formular prüfen
- Braucht das Formular (fKraft/fEngine/etc.) neue Felder?
- Braucht `getEnts()` eine Anpassung für den neuen KPI?

### Schritt 4 – Diff zeigen, Bestätigung abwarten

### Schritt 5 – Implementieren
- KPI in `KPIS.[system]` Array eintragen
- Falls nötig: Formular anpassen
- Falls nötig: `getEnts()` anpassen
- INFO-Eintrag hinzufügen wenn KPI Erklärung braucht

## Wichtige Regeln
- `k` darf nicht mit bestehendem Key kollidieren
- `hb` korrekt setzen – falsch = PR-Erkennung invertiert
- Neue KPIs brauchen in der Regel KEINE Migration (leere History ist OK)
- Unlock-abhängige KPIs: `isUnlocked()` Logik prüfen
